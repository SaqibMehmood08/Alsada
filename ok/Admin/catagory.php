...
<body>
  ...
  <div class="container-fluid">
    <a  class="navbar-brand" href="/completed Web/blog/index.php">AL-SADA</a>
    ...
    <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
      <div class="navbar-nav">
        ...
        <a class="nav-link" href="/completed Web/blog/post.php">Design</a>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
  ...
  $result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `measure`");
  $total_records = mysqli_fetch_array($result_count);
  $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
  $second_last = $total_no_of_pages - 1; // total page minus 1

    $result = mysqli_query($con,"SELECT * FROM `measure`ORDER BY ID DESC LIMIT $offset, $total_records_per_page");
    while($row = mysqli_fetch_array($result)){
      ?>
   <tr>
        <td><?php echo $row['phone'];?></td>
        <td><?php echo $row['customer_name'];?></td>
        <td><?php echo $row['accept_date'];?></td>
          <td><?php echo $row['deliver_date'];?></td>
        <td><a href="print_measure.php?id= <?php echo $row['id'];?>">Update</a></td>
